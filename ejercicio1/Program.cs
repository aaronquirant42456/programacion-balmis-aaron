...
namespace Ejercicio1
{
    ...
    public class Biblioteca
    {
        ...
        {
            // List<T>.find espera un predicado que devuelva bool por tanto usamos Predicate<Libro>
            // y miramos cada libro y comprobamos si su isbn es igual al que se nos pasa como parámetro
        ...
        public void Presta(string dni, string isbn)
        {
            ...
            var prestamo = (dni, libro.Titulo, isbn);
            Prestamos.Add(prestamo.ToString());
        }
        public bool EstaPrestado(string isbn) => Prestamos.Contains(isbn);
        public int CuentaLibrosConNumeroDePaginasMenorA(int num) => Libros.Count(libro => libro.NumPaginas < num);
        public void EliminaPorAutor(string autor) => Libros.RemoveAll(libro => libro.Autor == autor);
        public LibroResumen AutorTitulo(string isbn)
        {
            Libro buscar = Libros.Find(libro => libro.ISBN == isbn)!;
            return new LibroResumen(buscar.Titulo, buscar.Autor);
        }
    }
}
